...
class ColorDataStore {
    ...
    var colors = []
    static let sharedDataStore = ColorDataStore()
    ...
    func fetchData ()
    {
        ...
        let colorFetch = NSFetchRequest(entityName: SavedColor.entityName)
        
        let createdAtSort = NSSortDescriptor(key: "createdAt", ascending: true)

        
        colorFetch.sortDescriptors = [createdAtSort]
        do {
            colors = try managedObjectContext.executeFetchRequest(colorFetch) as! [SavedColor] // the as [color] is casting it to type array of messages.
        } catch {
    ...
    func generate(contentString: String) {
        let color = NSEntityDescription.insertNewObjectForEntityForName(SavedColor.entityName, inManagedObjectContext: self.managedObjectContext) as! SavedColor
